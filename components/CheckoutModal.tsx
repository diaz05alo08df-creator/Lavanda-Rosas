          ...
          {step === 'form' ? (
            <>
              <div className="bg-brand-dark px-4 py-3 sm:px-6 flex justify-between items-center">
                <h3 className="text-lg leading-6 font-medium text-white font-serif">Finalizar Compra</h3>
                <button onClick={handleCloseModal} className="text-white hover:text-gray-200"><X size={24} /></button>
              </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4 max-h-[70vh] overflow-y-auto">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <label className={`cursor-pointer border rounded-lg p-4 flex flex-col items-center gap-2 transition ${formData.deliveryMethod === 'shipping' ? 'border-brand-primary bg-purple-50 ring-1 ring-brand-primary' : 'border-gray-200 hover:border-brand-primary/50'}`}>
                        <input type="radio" name="deliveryMethod" value="shipping" checked={formData.deliveryMethod === 'shipping'} onChange={handleChange} className="sr-only"/>
                      ...
                      <label className={`cursor-pointer border rounded-lg p-4 flex flex-col items-center gap-2 transition ${formData.deliveryMethod === 'pickup' ? 'border-brand-primary bg-purple-50 ring-1 ring-brand-primary' : 'border-gray-200 hover:border-brand-primary/50'}`}>
                        <input type="radio" name="deliveryMethod" value="pickup" checked={formData.deliveryMethod === 'pickup'} onChange={handleChange} className="sr-only"/>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="col-span-1 md:col-span-2">
                      ...
                      <input type="text" name="fullName" required value={formData.fullName} onChange={handleChange} className="mt-1 bg-white text-gray-900 focus:ring-brand-primary focus:border-brand-primary block w-full border-gray-300 rounded-md py-2.5 px-3 border shadow-sm"/>
                    </div>
                    {formData.deliveryMethod === 'shipping' && (
                      <div className="col-span-1 md:col-span-2 animate-fade-in">
                        ...
                        <div className="mt-1 relative rounded-md shadow-sm">
                          ...
                          <input type="text" name="address" required={formData.deliveryMethod === 'shipping'} placeholder="Calle, Número, Colonia, Ciudad" value={formData.address} onChange={handleChange} className="bg-white text-gray-900 focus:ring-brand-primary focus:border-brand-primary block w-full pl-10 border-gray-300 rounded-md py-2.5 px-3 border"/>
                        </div>
                      </div>
                    )}
                    <div>
                      ...
                      <div className="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="tel" name="phone" required value={formData.phone} onChange={handleChange} className="bg-white text-gray-900 focus:ring-brand-primary focus:border-brand-primary block w-full pl-10 border-gray-300 rounded-md py-2.5 px-3 border"/>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="date" name="deliveryDate" required value={formData.deliveryDate} onChange={handleChange} min={new Date().toISOString().split('T')[0]} className="bg-white text-gray-900 focus:ring-brand-primary focus:border-brand-primary block w-full pl-10 border-gray-300 rounded-md py-2.5 px-3 border"/>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2 mt-2">
                      ...
                      <div className="relative rounded-md shadow-sm">
                        <div className="absolute top-3 left-3 flex items-start pointer-events-none"><MessageSquare className="h-5 w-5 text-gray-400" /></div>
                        <textarea name="dedication" rows={2} value={formData.dedication} onChange={handleChange} placeholder="Escribe un mensaje especial..." className="bg-white text-gray-900 focus:ring-brand-primary focus:border-brand-primary block w-full pl-10 border-gray-300 rounded-md py-2 px-3 border"/>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2 mt-2">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <label className={`cursor-pointer border rounded-lg p-4 flex items-center gap-3 transition ${formData.paymentMethod === 'card' ? 'border-brand-primary bg-purple-50 ring-1 ring-brand-primary' : 'border-gray-200 hover:border-brand-primary/50'}`}>
                          ...
                          <div className={`p-2 rounded-full ${formData.paymentMethod === 'card' ? 'bg-brand-primary text-white' : 'bg-gray-100 text-gray-500'}`}><CreditCard size={20} /></div>
                          <div><span className="font-bold text-sm text-gray-800 block">Tarjeta</span><span className="text-xs text-gray-500">Pago seguro online</span></div>
                        </label>
                        <label className={`cursor-pointer border rounded-lg p-4 flex items-center gap-3 transition ${formData.paymentMethod === 'cash' ? 'border-brand-primary bg-purple-50 ring-1 ring-brand-primary' : 'border-gray-200 hover:border-brand-primary/50'}`}>
                          ...
                          <div className={`p-2 rounded-full ${formData.paymentMethod === 'cash' ? 'bg-brand-primary text-white' : 'bg-gray-100 text-gray-500'}`}><Banknote size={20} /></div>
                          <div><span className="font-bold text-sm text-gray-800 block">Efectivo</span><span className="text-xs text-gray-500">Paga al recibir</span></div>
                        </label>
                      </div>
                    </div>
                  </div>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse items-center justify-between">
                  <button type="submit" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-6 py-3 bg-brand-primary text-base font-medium text-white hover:bg-brand-dark focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                    Confirmar Orden (${total.toLocaleString('es-MX')})
                  </button>
                  <div className="mt-3 sm:mt-0 text-xs text-gray-400 text-center sm:text-left">* Al confirmar, aceptas nuestros términos.</div>
                </div>
              </form>
            </>
          ) : (
            <div className="bg-white flex flex-col items-center">
              <div className="bg-green-500 w-full py-4 text-center text-white relative">
                 <h3 className="text-xl font-bold flex items-center justify-center gap-2"><CheckCircle size={28} /> ¡Pedido Confirmado!</h3>
                 <button onClick={handleCloseModal} className="absolute top-4 right-4 text-white/80 hover:text-white"><X size={24} /></button>
              </div>
              <div className="p-8 w-full max-w-md bg-white">
                <div className="border-2 border-dashed border-gray-300 p-6 rounded-lg bg-gray-50 font-mono text-sm relative">
                  ...
                  <div className="border-t-2 border-dashed border-gray-300 pt-4 mb-4">
                    <div className="flex justify-between items-center text-lg font-bold"><span>TOTAL</span><span>${total.toLocaleString('es-MX')}</span></div>
                    <div className="flex justify-between items-center text-xs text-gray-500 mt-1"><span>Método de Pago:</span><span className="uppercase">{formData.paymentMethod === 'card' ? 'Tarjeta' : 'Efectivo'}</span></div>
                  </div>
                  ...
                  <div className="text-center text-xs text-gray-400 mt-4"><p>¡Gracias por tu preferencia!</p></div>
                </div>
                <div className="mt-6 flex gap-3">
                  <button onClick={handlePrint} className="flex-1 py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg font-medium flex items-center justify-center gap-2 transition"><Printer size={18} /> Imprimir</button>
                  <button onClick={handleCloseModal} className="flex-1 py-3 bg-brand-dark hover:bg-brand-primary text-white rounded-lg font-medium transition">Cerrar</button>
                </div>
              </div>
            </div>
          )}
