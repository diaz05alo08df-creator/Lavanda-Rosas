          ...
          <div className="grid grid-cols-1 lg:grid-cols-12">
            ...
            <div className="lg:col-span-5 p-8 md:p-12 text-white flex flex-col justify-center bg-white/5">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-bold mb-4">
                Asistente Floral Inteligente
              </h2>
              <p className="text-purple-100 text-lg mb-8">
                Utilizamos inteligencia artificial avanzada para ayudarte a expresar tus sentimientos y encontrar el regalo perfecto.
              </p>
              
              <div className="flex flex-col gap-3">
                <button 
                  onClick={() => setActiveTab('recommendation')}
                  className={`flex items-center gap-3 p-4 rounded-xl transition-all text-left ${activeTab === 'recommendation' ? 'bg-white text-brand-dark shadow-lg' : 'bg-white/10 hover:bg-white/20 text-purple-100'}`}
                >
                  <div className={`p-2 rounded-full ${activeTab === 'recommendation' ? 'bg-purple-100 text-brand-primary' : 'bg-brand-dark'}`}>
                    <Gift size={20} />
                  </div>
                  <div>
                    <h3 className="font-bold">Sugerir Regalo Ideal</h3>
                    <p className="text-xs opacity-80">Encuentra el arreglo perfecto</p>
                  </div>
                </button>

                <button 
                  onClick={() => setActiveTab('message')}
                  className={`flex items-center gap-3 p-4 rounded-xl transition-all text-left ${activeTab === 'message' ? 'bg-white text-brand-dark shadow-lg' : 'bg-white/10 hover:bg-white/20 text-purple-100'}`}
                >
                  <div className={`p-2 rounded-full ${activeTab === 'message' ? 'bg-purple-100 text-brand-primary' : 'bg-brand-dark'}`}>
                    <PenTool size={20} />
                  </div>
                  <div>
                    <h3 className="font-bold">Redactor de Dedicatorias</h3>
                    <p className="text-xs opacity-80">Crea el mensaje perfecto</p>
                  </div>
                </button>
              </div>
            </div>
                ...
                <div className="animate-fade-in">
                   ...
                   <form onSubmit={handleRecommendationSubmit} className="space-y-5">
                      <div>
                        ...
                        <select 
                          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary py-3 px-4 bg-white text-gray-900"
                          value={recOccasion}
                      ...
                      <div>
                        ...
                        <textarea 
                          placeholder="Ej. Le encantan los tulipanes, su color favorito es el rosa, es una persona muy elegante..."
                          rows={3}
                          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary py-3 px-4 bg-white text-gray-900"
                      ...
                      <button 
                        type="submit" 
                        disabled={isLoading || !recOccasion || !recPreferences}
                        className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-brand-dark hover:bg-brand-primary disabled:opacity-50 transition"
                      >
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mt-2 flex justify-between items-center">
                                ...
                                <button 
                                  ...
                                  className="text-xs bg-brand-dark text-white px-3 py-1.5 rounded-full hover:bg-brand-primary transition"
                                >
                                  Agregar al Carrito
                                </button>
                              </div>
                ...
                <div className="animate-fade-in">
                  ...
                  <form onSubmit={handleMessageSubmit} className="space-y-5">
                    <div>
                      ...
                      <select 
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary py-3 px-4 bg-white text-gray-900"
                        value={msgData.occasion}
                        onChange={(e) => setMsgData({...msgData, occasion: e.target.value})}
                      >
                        <option value="">Selecciona...</option>
                        <option value="Cumpleaños">Cumpleaños</option>
                        <option value="Aniversario">Aniversario</option>
                    ...
                    <div>
                      ...
                      <input 
                        type="text" 
                        placeholder="Ej. Mi esposa, Mi mamá..."
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-primary focus:ring-brand-primary py-3 px-4 bg-white text-gray-900"
                        value={msgData.recipient}
                        onChange={(e) => setMsgData({...msgData, recipient: e.target.value})}
                      />
                    </div>
                      ...
                      <div className="mt-2 flex gap-2 flex-wrap">
                        {['Romántico', 'Divertido', 'Formal', 'Poético'].map((tone) => (
                          <button
                            key={tone}
                            type="button"
                            onClick={() => setMsgData({...msgData, tone})}
                    ...
                    <button 
                      type="submit" 
                      disabled={isLoading || !msgData.occasion || !msgData.recipient}
                      className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-brand-dark hover:bg-brand-primary disabled:opacity-50 transition"
                    >
                  ...
                  {generatedMessage && (
                    <div className="mt-6 p-4 bg-yellow-50 rounded-lg border border-yellow-100 relative group">
                      ...
                      <button 
                        ...
                        className="absolute top-2 right-2 p-2 text-gray-400 hover:text-brand-dark rounded bg-white/50 hover:bg-white transition"
