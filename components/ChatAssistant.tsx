...
const ChatAssistant: React.FC = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col h-[500px] animate-fade-in-up">
          {/* Header */}
          <div className="bg-brand-dark p-4 flex justify-between items-center text-white">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-bold text-sm">Rosa - Asistente IA</h3>
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-gray-100 flex gap-2">
            <input
              ...
              className="flex-1 bg-white text-gray-900 border border-gray-200 rounded-full px-4 py-2 text-sm focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary"
            />
            <button 
              type="submit" 
              disabled={!input.trim()}
              className="p-2 bg-brand-primary text-white rounded-full hover:bg-brand-dark disabled:opacity-50 transition"
            >
              <Send size={18} />
            </button>
          </form>
